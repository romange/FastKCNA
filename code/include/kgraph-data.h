...
namespace kgraph
{
    ...
    public:
        ...
        void copyelement(unsigned i, unsigned j)
        {
            if (i == j)
            {
                return;
            }
            char *data_i = data + stride * i;
            char *data_j = data + stride * j;
            memcpy(data_i, data_j, stride);
        }
...
#pragma omp parallel for
        ...
        void load(const std::string &path, unsigned dim, unsigned skip = 0, unsigned gap = 0, unsigned *file2data = nullptr)
        {
        ...
        void load_lshkit(std::string const &path, unsigned *file2data = nullptr)
        {
            ...
            if (!is)
                throw io_error(path);
            if (header[0] != sizeof(T))
                throw io_error(path);
            is.close();
            unsigned D = header[2];
            unsigned skip = LSHKIT_HEADER * sizeof(unsigned);
            unsigned gap = 0;
            load(path, D, skip, gap, file2data);
        }
...
#pragma omp parallel for
        ...
        virtual unsigned getPerDimSizeof() const
        {
            return sizeof(proxy[0][0]);
        }
    ...
    inline float AverageRecall(Matrix<float> const &gs, Matrix<float> const &result, unsigned K = 0)
    {
        ...
        if (!(gs.dim() >= K))
            throw invalid_argument("gs.dim() >= K");
        if (!(result.dim() >= K))
            throw invalid_argument("result.dim() >= K");
        if (!(gs.size() >= result.size()))
            throw invalid_argument("gs.size() > result.size()");
        float sum = 0;
